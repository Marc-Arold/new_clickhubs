...
export default function PronosPalmaresPage() {
  ...
  const leaderboard = pronosLeaderboardMock.map((p) => {
    const propGain =
      totalOddsSum > 0 ? Math.round((p.totalOdds / totalOddsSum) * netPool) : 0;
  ...
  return (
    <div className="max-w-4xl space-y-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={() => navigate("/pronos")}
          className="text-gray-500 hover:text-white text-xs bg-transparent border-none cursor-pointer p-0"
        >
          Lòbi Pronos
        </button>
      </div>
      ...
      <div className="bg-dark-surface border border-white/10 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-gray-400 text-xs">
          Klasman baze sou{" "}
          <span className="text-white font-medium">kote total fich</span> chak
          jwè. Genyan resevwa pati pwopòsyonèl: pi gwo kote = pi gwo share.
          Chifè "Gain Potansyèl" yo montre distribisyon si tout match kòrèk jodi
          a.
        </p>
      </div>
      ...
      {userRow && (
        <div className="bg-gold/10 border border-gold/30 rounded-2xl p-4">
          <p className="text-gold font-bold text-xs uppercase tracking-wide mb-2">
            Pozisyon Ou
          </p>
          <div className="flex items-center gap-3">
            <span className="text-lg w-8 text-center shrink-0">
              {userRow.rank <= 3
                ? MEDALS[userRow.rank - 1]
                : `#${userRow.rank}`}
            </span>
            <div className="flex-1">
              ...
              <p className="text-gray-400 text-xs">
                ...
                <span className="text-white font-bold">
                  {userRow.totalOdds.toFixed(1)}
                </span>{" "}
                · {userRow.ficheEvents} evènman
              </p>
            </div>
            <div className="text-right">
              <p className="text-gold font-black text-base">
                {userRow.displayGain.toLocaleString()} HTG
              </p>
              <p className="text-gray-500 text-[10px]">Gen potansyèl</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-dark-surface border border-white/10 rounded-2xl overflow-hidden">
        <div className="px-5 py-3 border-b border-white/10 flex items-center justify-between">
          ...
          <p className="text-gray-500 text-xs">
            {pronosLeaderboardMock.length} jwè
          </p>
        </div>
        ...
        <div className="grid grid-cols-[auto_1fr_auto_auto] gap-3 px-5 py-2 border-b border-white/5 text-[10px] text-gray-600 uppercase tracking-wider font-bold">
          ...
          <span className="text-right hidden sm:block">Gen potansyèl</span>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <p
                    className={`text-sm font-medium truncate ${p.isUser ? "text-gold" : "text-white"}`}
                  >
                    {p.username}{" "}
                    {p.isUser && (
                  ...
                  <p className="text-gray-600 text-[10px]">
                    {p.ficheEvents} evènman · {p.ficheCount} fich
                  </p>
                </div>
                ...
                <div className="text-right">
                  <p
                    className={`text-sm font-bold ${p.status === "lost" ? "text-gray-600 line-through" : p.isUser ? "text-gold" : "text-white"}`}
                  >
                    ×{p.totalOdds.toFixed(1)}
                  </p>
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${STATUS_STYLES[p.status]}`}
                  >
                ...
                <div className="text-right hidden sm:block">
                  {p.status === "lost" ? (
                    ...
                    <p
                      className={`text-sm font-bold ${p.isUser ? "text-gold" : "text-gray-300"}`}
                    >
