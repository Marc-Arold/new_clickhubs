  ...
  const filteredSplits = useMemo(() => {
    ...
    switch (activeSort) {
      case 'odds':
        ...
        break
      case 'slots':
        splits = splits.sort((a, b) => (b.maxPlayers - b.currentPlayers) - (a.maxPlayers - a.currentPlayers))
        break
      case 'price_asc':
        splits = splits.sort((a, b) => a.playerEntryFee - b.playerEntryFee)
        break
      case 'newest':
        splits = splits.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        break
    }
  ...
  return (
    <div className="max-w-6xl space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Bank Pari</h1>
      ...
      <p className="text-gray-500 text-sm">
        {filteredSplits.length} split{filteredSplits.length !== 1 ? ' yo' : ''} disponib
      </p>
      ...
      {filteredSplits.length > 0 ? (
        ...
        <div className="text-center py-16 bg-dark-surface border border-white/10 rounded-2xl">
          <p className="text-gray-400 text-sm">Pa gen split ki koresponn ak filtè ou yo.</p>
          <button
            onClick={() => { setActiveStatus('all'); setActiveSort('odds') }}
