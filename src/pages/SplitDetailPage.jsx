...
import { ArrowLeft, Landmark, Users, TrendingUp, Check, Shield, ShieldAlert } from 'lucide-react'
...
export default function SplitDetailPage() {
    ...
    return (
      <div className="max-w-4xl space-y-6">
        <div className="text-center py-16 bg-dark-surface border border-white/10 rounded-2xl">
          ...
          <button onClick={() => navigate('/bank-pari')} className="mt-4 text-gold text-sm font-medium hover:underline cursor-pointer bg-transparent border-none">
            Retounen nan Bank Pari
          </button>
        </div>
      </div>
    )
  ...
  const isParticipant = split.participants.some(p => p.username === '@ClicBoss') || joined
  ...
  const canJoin = split.status === 'open' && !isCreator && !isParticipant && slotsLeft > 0
  ...
  const creatorProfit = anyEventLost
    ? Math.round(split.currentPlayers * split.playerEntryFee * 0.90)
    : 0
  const creatorLoss = allEventsWon
    ? split.currentPlayers * potentialPayout
    : 0
  ...
  async function handleJoin() {
    ...
    try {
      ...
      updateBalance(user.availableBalance - split.playerEntryFee, user.escrowedBalance + split.playerEntryFee)
      setJoined(true)
    } catch {
      // API error
    }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      {/* Header */}
      <div>
        <button onClick={() => navigate('/bank-pari')} className="flex items-center gap-1 text-gray-400 hover:text-white text-sm mb-3 bg-transparent border-none cursor-pointer p-0">
          <ArrowLeft size={16} /> Retounen nan Bank Pari
        </button>
        ...
        <p className="text-gray-400 text-sm mt-1">
          {isCreator ? 'Ou se Bankè nan split sa a.' : 'Wè detay split sa a epi antre si ou vle.'}
        </p>
      </div>
      ...
      {(isParticipant && !isCreator) && (
        <div className="bg-success/10 border border-success/20 rounded-xl p-3 flex items-center gap-2">
          ...
          <span className="text-success font-bold text-sm">Ou nan split sa a!</span>
        </div>
      )}
      ...
      <div className="bg-dark-surface border border-white/10 rounded-2xl p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-full bg-gold/10 flex items-center justify-center">
              <span className="text-gold text-sm font-bold">{split.creatorInitials}</span>
            </div>
            <div>
          ...
          <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
            split.status === 'open' ? 'bg-success/20 text-success' :
            split.status === 'live' ? 'bg-warning/20 text-warning' :
            split.status === 'completed' ? 'bg-white/10 text-gray-400' :
            'bg-white/10 text-gray-400'
          }`}>
            {split.status === 'open' ? 'Ouvri' :
             split.status === 'live' ? 'Live' :
             split.status === 'completed' ? 'Fini' :
             split.status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center p-2 bg-white/5 rounded-lg">
            ...
            <p className="text-gold font-bold text-lg">{split.totalOdds.toFixed(2)}x</p>
          </div>
          <div className="text-center p-2 bg-white/5 rounded-lg">
            <p className="text-gray-500 text-xs">Antre</p>
            <p className="text-white font-bold text-lg">{(split.playerEntryFee / 1000).toFixed(0)}K</p>
          </div>
          <div className="text-center p-2 bg-white/5 rounded-lg">
            <p className="text-gray-500 text-xs">Jwè</p>
            <p className="text-white font-bold text-lg">{split.currentPlayers}/{split.maxPlayers}</p>
          </div>
        </div>
        ...
        <div>
          <div className="w-full bg-white/5 rounded-full h-2">
            <div
              ...
              style={{ width: `${Math.round((split.currentPlayers / split.maxPlayers) * 100)}%` }}
            />
          </div>
      ...
      <div className="bg-gold/5 border border-gold/10 rounded-xl p-4 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-gold font-semibold text-sm">Gen Potansyèl pou Jwè</h3>
        </div>
        <div className="flex items-end gap-2">
          <span className="text-gray-400 text-sm">{split.playerEntryFee.toLocaleString()} HTG</span>
          <span className="text-gray-600">→</span>
          <span className="text-gold font-bold text-xl">{potentialPayout.toLocaleString()} HTG</span>
        </div>
        <p className="text-gray-500 text-xs">Si tout {split.events.length} match kòrèk</p>
      </div>
      ...
      <div className="bg-dark-surface border border-white/10 rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-white">Jwè ({split.currentPlayers})</h3>
        </div>
        <div className="flex flex-wrap gap-2">
          {split.participants.slice(0, 12).map((p) => (
            <span key={p.username} className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-full text-xs text-gray-400">
              {p.username}
            </span>
          ))}
      ...
      {canJoin && !joined && (
        ...
        >
          {joining ? 'Ap antre...' : `Antre nan Split — ${split.playerEntryFee.toLocaleString()} HTG`}
        </button>
      )}
      ...
      {canJoin && split.playerEntryFee > user.availableBalance && (
        <div className="flex items-center gap-2 bg-danger/10 border border-danger/20 text-danger text-sm px-3 py-2 rounded-lg">
          <ShieldAlert size={14} /> Ou pa gen ase lajan. Balans: {user.availableBalance.toLocaleString()} HTG
        </div>
      )}
      ...
      {joined && (
        <div className="bg-success/10 border border-success/20 rounded-xl p-4 text-center space-y-2">
          ...
          <p className="text-gray-400 text-sm">Lajan an an eskwo. Ap tann rezilta match yo.</p>
        </div>
      )}
      ...
      {isCreator && split.status === 'completed' && (
        <div className={`border rounded-xl p-4 text-center space-y-2 ${
          anyEventLost ? 'bg-success/10 border-success/20' : 'bg-danger/10 border-danger/20'
        }`}>
          {anyEventLost ? (
            <>
              <p className="text-success font-bold">Ou Genyen!</p>
              <p className="text-success text-lg font-bold">{creatorProfit.toLocaleString()} HTG pwofi</p>
              <p className="text-gray-400 text-xs">Apre 10% frè platfòm</p>
            </>
          ) : (
            <>
              <p className="text-danger font-bold">Jwè yo Genyen</p>
              <p className="text-danger text-lg font-bold">Ou peye {creatorLoss.toLocaleString()} HTG</p>
              <p className="text-gray-400 text-xs">bay {split.currentPlayers} jwè</p>
            </>
          )}
        </div>
      )}
