...
import { ArrowLeft, Landmark, Check, DollarSign, AlertCircle } from 'lucide-react'
...
export default function CreateSplitPage() {
  ...
  const allOddsValid = selectedEvents.length >= 2 && selectedEvents.every(e => {
    const o = parseFloat(odds[e.id])
    return o && o >= 1.1
  })

  const allPicksValid = selectedEvents.length >= 2 && selectedEvents.every(e => picks[e.id])

  const netPayoutPerPlayer = entryFeeNum > 0 ? Math.round(entryFeeNum * totalOdds) - entryFeeNum : 0
  const maxPlayers = netPayoutPerPlayer > 0 ? Math.floor(escrowNum / netPayoutPerPlayer) : 0

  const isValid = selectedEvents.length >= 2 &&
    allOddsValid &&
    allPicksValid &&
    entryFeeNum >= 500 &&
    escrowNum >= 10000 &&
  ...
  async function handleCreate() {
    try {
      ...
      updateBalance(user.availableBalance - escrowNum, user.escrowedBalance + escrowNum)
      ...
      <div className="max-w-4xl space-y-6">
        <div className="text-center py-8 space-y-4">
          ...
          <p className="text-gray-400 text-sm">
            Split <span className="text-gold font-bold">{totalOdds.toFixed(2)}x</span> sou{' '}
            <span className="text-white font-medium">{selectedEvents.length} match</span> kreye avèk siksè.
          </p>
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Chwazi match yo, mete kòt ou, epi atire jwè yo.</p>
      </div>

      {/* Step 1: Event selection */}
      <MultiEventSelector selectedEvents={selectedEvents} onEventsChange={setSelectedEvents} />

      {/* Step 2: Pick for each event */}
      {selectedEvents.length >= 2 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-white">2. Chwa ou pou chak match</h3>
          <div className="space-y-2">
            {selectedEvents.map((match) => (
              <div key={match.id} className="bg-dark-surface border border-white/10 rounded-xl p-3">
                <p className="text-white text-sm font-medium mb-2">{match.home} vs {match.away}</p>
      ...
      {/* Step 3: Odds */}
      {allPicksValid && (
        <OddsInput events={selectedEvents} odds={odds} onOddsChange={handleOddsChange} picks={picks} />
      )}
      ...
      {allPicksValid && allOddsValid && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">Konbyen chak jwè dwe peye pou antre nan split ou a.</p>
          ...
          {entryFeeNum >= 500 && (
            <div className="bg-dark-surface border border-white/10 rounded-xl p-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white">{entryFeeNum.toLocaleString()} HTG</span>
              </div>
              <div className="flex justify-between text-sm mt-1">
                <span className="text-gray-400">Jwè ka genyen</span>
                <span className="text-gold font-bold">{Math.round(entryFeeNum * totalOdds).toLocaleString()} HTG</span>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Review & Create */}
      {allPicksValid && allOddsValid && entryFeeNum >= 500 && escrowNum >= 10000 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-white">6. Revize & Pibliye</h3>
          <div className="bg-dark-surface border border-white/10 rounded-xl p-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Match</span>
              <span className="text-white">{selectedEvents.length} match</span>
            </div>
            <div className="flex justify-between text-sm">
